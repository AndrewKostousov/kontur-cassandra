    ...
    public class AllBoxEventSeriesWriter : IDisposable
    {
        ...
        public Timestamp Write([NotNull] ProtoBoxEvent protoBoxEvent)
        {
            ...
            try
            {
                ...
                throw new InvalidProgramStateException(string.Format("Failed to write event in {0} attempts: {1}", maxAttemptsCount, protoBoxEvent));
            }
            finally
            {
                sw.Stop();
                if(sw.Elapsed >= TimeSpan.FromSeconds(10))
                    Log.For(this).Warn(string.Format("[LONG DELAY] AllBoxEventSeriesWriter.Write() took {0} for: {1}", sw.Elapsed, protoBoxEvent));
            }
        }
