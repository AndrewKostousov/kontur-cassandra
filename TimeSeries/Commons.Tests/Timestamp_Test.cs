...
namespace Commons.Tests
{
    ...
    public class TimestampTest
    {
        ...
        public void MinMaxValues()
        {
            ...
            Assert.Throws<ArgumentOutOfRangeException>(() => { var t = Timestamp.MaxValue + TimeSpan.FromTicks(1); });
            Assert.Throws<ArgumentOutOfRangeException>(() => { var t = Timestamp.MinValue - TimeSpan.FromTicks(1); });
            Assert.Throws<ArgumentOutOfRangeException>(() => { var t = new Timestamp(-1); });
            Assert.Throws<ArgumentOutOfRangeException>(() => { var t = new Timestamp(DateTime.MinValue.Ticks - 1); });
            Assert.Throws<ArgumentOutOfRangeException>(() => { var t = new Timestamp(DateTime.MaxValue.Ticks + 1); });
            Assert.Throws<ArgumentOutOfRangeException>(() => { var t = new Timestamp(3155378975999999999L + 1); });
        }
